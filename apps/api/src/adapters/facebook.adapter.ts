      ...
      if (convResponse.ok) {
        ...
        if (dataArr?.length) {
          ...
          if (user?.name) {
            ...
            // Use direct Graph API picture URL — browser will handle the redirect
            const profilePic = `https://graph.facebook.com/${userId}/picture?type=large&access_token=${pageAccessToken}`;
